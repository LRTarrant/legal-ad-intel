...
Last updated: 2026-06-26 (Media Consumption baseline + page shipped & prod-verified, PRs #467–#475; Impeccable critique 34→38; P3 cosmetics parked below)
...
- **State Legal News banner (single-incident PI) — research done, build not started.** National state-page surface for single-incident PI news (MVA, trucking, motorcycle, construction, sexual abuse, TBI, explosions; NO mass torts), Alabama as the template then all 50 states. Sources must be republishable: government data (OSHA state filter, existing FARS, CourtListener) + Google News headlines via SearchApi/Apify + own AI summaries. No paid verdict DB can be republished (AJVR/VerdictSearch/Law360 are research-only). Design (card carousel) being done in Claude Code w/ Impeccable. Full source stack, query template, proposed `state_legal_news` table, and the relevance-filter note are in `memory.md` → "Feature research — State Legal News banner." Pull into active work when PI state pages are demo-stable.
- **Strategy Engine (PI media-strategy recommendations) — research done, pre-council, build not started.** State-page feature (Alabama first, PI-specific) that turns surfaced market data + a few positioning inputs into 2-3 reasoned media strategies, exportable to PDF/slides, serving firms + agencies + media sellers. Architecture = rules-and-data scaffold (fixed strategy archetypes: Head-to-Head / Niche Carve-Out / Audience Play / Always-On / Surge) with AI as writer, not strategist; Competitive Analysis data selects the archetype, demographic baseline + format-genre media CSV map audience→channel→funnel. Demographic baseline researched: **Pew (republishable w/ attribution) + BLS ATUS (public domain)** as the build layer; Nielsen is research-only (walled). Media CSV saved at `docs/Media_Company_Lookup.xlsx` (DMA-level, Format_Genre is the audience key; Media-Monitors-sourced so rural markets need FCC gap-fill). Lance to run model-council skill against the 5-question thesis before building. Full detail + council thesis in `memory.md` → "Feature research — Strategy Engine."
- **Media Consumption page — P3 cosmetic polish (deferred; shipped at 38/40, no P0/P1/P2).** The `/media-consumption` surface (PRs #467–#475) is ship-grade; these are the only open cosmetics from the Impeccable critique, none blocking:
  ...
  - **11px attribution text** (source links + "cited as fact") is small; bump to 12px (`text-xs`) if it reads tight.
  - **"cited as fact"** repeats per metric row on cite-as-fact channels (radio); could consolidate to one marker per channel.
