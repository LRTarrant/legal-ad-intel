...
- **2026-06-30 — Strategy Engine competitive field + white-space polish (PR #535, merged + prod-verified).** Four additive UI improvements, no schema. (1) Firm names in the competitive field now **link out** to their site (carrying the `advertiser_domain` that `get_pi_competitors_by_dma` already returns but was dropped in grouping), plain text when none. (2) **Named the ranking source honestly** — it's `pi_search` only (breadth→density); the prior "ad library" tag was an overclaim, removed + labeled "Ranked across paid-search presence (pi_search, per DMA)." (3) **Show past 6** — top 6 default + Show-all (up to 20) toggle. (4) The Paid-search white-space channel can **reveal its active firms** ("who?") from the same pi_search roster; SEO stays count-only (different serp roster, not on payload — follow-up), modeled channels stay modeled. Prod-verified on AL/Birmingham (0 console errors). **Data lesson:** competitor presence ranking is pi_search-only; ad-library/YouTube feeds the *creative* section, not the ranking.
- **2026-06-30 — PI economics layer v1: "What your budget buys" (PR #533, merged + prod-verified).** Fulfills the intake rail's long-unfulfilled "Realistic cost per signed case for [tort]" promise. New `pi_ad_economics` table (grain `case_type × market_tier`; CPC range, click-to-lead + lead-to-signed presets, case value + catastrophic tail, contingency, **per-metric provenance + confidence**, **RLS read-only public**) + read RPC. Pure TS funnel module `web/lib/strategy-engine/economics.ts` computes spend → clicks → leads → signed → cost-per-case and returns the **full stage breakdown**; wired into `api/strategy/generate` and rendered as a cost section in the deck. **v1 design:** CPC + case value are fixed picks; **click-to-lead + lead-to-signed are user-adjustable levers** (the two softest inputs — adjustability is the honesty mechanic). Default leads with the **lever envelope** (elite vs poor intake spread), not a single number, with **fee-per-case inline** as ROI context. **Funnel math in TS not SQL** (diverges from the MT lookup) so levers recompute live + a v2 has per-stage access. Benchmark = 2026-06-30 research pass; key finding: keyword-level litigation CPCs, NOT the low WordStream "legal" category median. Migration reviewed clean; applied on merge. Prod-verified on AL/Birmingham (levers move the funnel live, trucking tail never "average", 0 console errors). **Post-merge todo:** regenerate `database.types.ts` (code reads via RPC + `any`, so non-blocking). Full detail → `memory.md`.
- **2026-06-30 — Strategy Engine: recommendation cards inherit measured whitespace read (PR #531, merged + prod-verified).** Fixed the cards showing `Whitespace: unmeasured` while the "Where the white space is" section below showed real firm counts — they read different sources for the same concept. Cards derived whitespace from `channel_competition_scores` (empty across all live markets → null → "unmeasured"); the section reads the `strategy_whitespace_channels` RPC (real counts). **Wiring fix, not new measurement:** routed the live strategist mapper through the existing `buildWhiteSpaceLink` in `recommendations.ts` (was dead code, now live + test-covered), keyed by `brief.tactic.channel`, using the `measured` array the route already fetched. Measured channels (paid_search, seo) now show the real status pill + count matching the section; modeled channels show honest "open/modeled", never a fabricated number; "unmeasured" appears nowhere. Added a one-place on-screen vocabulary legend (open/contested/defended + measured/modeled). **Known limit (by design):** `seo` isn't a `ChannelKey`, so the SEO+GBP card inherits the paid_search read; section keeps its standalone Organic SEO row. Prod-verified on AL/Birmingham (Google Search card "Defended · 22 firms active" = section). Full detail → `memory.md`.
- **2026-06-30 — Strategy Engine → Campaign Builder handoff, Phase 1 (PR #530, merged).** Widened the existing URL-param handoff so a user arriving from a Strategy deck lands in Campaign Builder ready to generate. **Additive only** — no draft record, no schema; **PI flow only** (Mass Tort handoff is a deliberate later phase). New `web/lib/strategy-engine/campaign-handoff.ts` is the single home for the contract (`budgetTierToRange`, `audienceToBuyerType`, `buildCampaignBuilderHandoff`). New link params: `market_display_name`, `firm_name`, `budget_tier`, `budget_min/max/midpoint`, `goal`. **Statewide is now first-class** — no DMA picked → `market_display_name="Statewide – <State>"`, `market_dma_code` omitted, and PIConfigForm enables Generate without a DMA (the PI router keys on the display name, not a DMA code). Unmapped case types (`nursing_home`, `workers_comp`) get an honest "coming soon" block instead of an empty `pi_category` dropdown. **Deferred:** `audience` is NOT passed in the URL (Builder's `buyer_type` comes from the subscription via `useFirms()` and is authoritative — map exists in lib + documented precedence, no override); `goal` is captured/stripped but has no PI input field yet. 6 node:test cases. **Post-merge browser checks to run:** (1) agency `firm_name` precedence over self-firm autofill, (2) unmapped-case-type block renders. Full detail → `memory.md`.
- **2026-06-30 — Strategy Engine intake redesigned to "The Analyst's Brief" (PR #527, merged + prod-verified).** Rebuilt the `/strategy` intake UI from the approved Claude Design template (imported via the claude_design MCP): a navy intelligence rail (typewriter live brief that retypes only from the point of change, picks-chips, and a deliverables list that FLIP-reorders to the user's selections with a "Top focus" marker) beside a calm **numbered** carded form (01–10) with budget cards, icon pills, a Yes/No/Unsure foundation check, and a sticky generate bar with a progress meter. Reduced-motion safe. **Backend contract untouched** — field values stay in slug vocab, the dynamic `/api/dma-markets` fetch + `/api/strategy/generate` payload + `<StrategyDeck>` render are all preserved; `goal_context` is now the 8th required field (matches the rail copy "Answer eight quick questions"). Swapped the design's `data-lucide`/`window.lucide` for `lucide-react`; added the design tokens/keyframes/`.lmi-mono` to `globals.css`; adapted the full-bleed `100vh` two-pane into a rounded panel that lives inside the `(app)` shell (no layout hacks). Two files changed (`strategy-client.tsx` + `globals.css`). Prod-verified on www.legalmarketingintelligence.com: TX→Houston interview, `POST /api/strategy/generate` 200, real Houston deck (FARS counties, real competitors, Houston radio, allocation = 100%, "no daytime TV" constraint honored), DMA refetch-on-state-change works, 0 console errors. This is the **UI** redesign; the **strategist-logic** redesign (gpt-5.5 grounded strategist) shipped earlier in PRs #496–#503.
...
### E. Strategy Engine → Campaign Builder handoff

**Goal:** Turn a generated Strategy deck into a launched campaign with as little re-entry as possible.

Status:
- **Shipped:** Phase 1 (#530) — widened URL-param contract; statewide is first-class; honest block for unmapped case types (nursing_home/workers_comp); additive, PI-flow only. Whitespace cards (#531) now inherit the measured channel read (no more "unmeasured").
- **Next:** **Phase 2** — persist a `strategy_run` object (save the generated strategy as a record so the handoff carries an ID instead of re-deriving from URL params). **Phase 3** — Mass Tort Strategy handoff + account-level entitlements (both deliberately deferred from Phase 1).
- **Deferred (Phase 1):** `audience` not passed in the URL (Builder's `buyer_type` comes from the subscription via `useFirms()` and is authoritative); `goal` is carried/stripped but has no PI input field yet.

### F. Strategy Engine economics (budget → signed cases)

**Goal:** Show a PI firm what its ad budget realistically buys, honestly, and let it see which lever changes the outcome.

Status:
- **Shipped v1 (#533):** `pi_ad_economics` table + pure TS funnel module + "What your budget buys" cost section (visible funnel, two live intake levers, per-number provenance, fee-per-case ROI context). CPC + case value fixed by `case_type × market_tier`; click-to-lead + lead-to-signed are the user levers.
- **Next (v2, noted — not built):** user enters a **target CPA** → tool shows the gap + which funnel stage to fix. The funnel module's full-stage return already supports it (no refactor); merges with the adjustable-lever idea.
- **Open follow-ups (non-blocking):** trucking median is a low-confidence placeholder estimate (needs a cleaner source); `boating` has no economics row (section omits); uncovered-case omit is code-verified, not browser-verified; regenerate `database.types.ts` for the new table/RPC.
