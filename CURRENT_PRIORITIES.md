...
Last updated: 2026-06-29 (Phase 3 legacy→v2 migration ✅ DONE — all 5 legacy state pages migrated + prod-verified, only Alabama stays bespoke, PRs #516/#518/#519/#520 — see §0/§5; Design D arc #507–#512 prior)
...
- **Phase 3 — ✅ DONE 2026-06-29 (TN #516, AZ #518, FL/GA/CA #519; docs #520).** All 5 legacy bespoke pages
  (AZ, CA, FL, GA, TN) migrated onto the v2 `[slug]` shell; the hand-written clients (~9k lines total) deleted
  and the legacy routes redirect to `/v2/<slug>`. Per Lance's lossless decision, GA's native FARS crash charts
  and CA's ped/bike case card were ported into the shell as optional, flag-gated features (plus FL/CA rate
  tiles), so no display was dropped. Each prod-verified. **Alabama stays bespoke** — it's the design
  source-of-truth the shared components were built from (converging it would be premature churn).
