...
Last updated: 2026-06-29 (State Intelligence "Design D" arc shipped + propagated to ~40 v2 states incl. the Legal & PI Viability reshape, PRs #507–#512 — see §0; Phase 3 legacy migration unblocked, §5)
...
- **2026-06-29 — Phase 3 legacy→v2 migration — IN PROGRESS (audit #514, backbone #515, TN #516 done + prod-verified; AZ/FL/GA/CA remaining).** Migrating the 5 custom legacy state pages (AZ/CA/FL/GA/TN) onto the shared v2 shell + deleting the bespoke clients. **Decisions (Lance): LOSSLESS; PORT GA's FARS crash charts + CA's ped/bike card.** Authoritative plan: **`docs/phase3-legacy-migration-audit.md`**. #515 added the optional `_types` backbone + a `customInsights[]` renderer + config-driven `partialYearLabels` (all guarded, ~40 live states unchanged). #516 migrated TN (config existed; redirect from the legacy route + deleted client; prod-verified). **The migration recipe + per-state remaining work + the state-specific shell features still to build (FL rate tiles, GA FARS crash section, CA ped/bike card) are in `memory.md`** — read it before resuming. Next: AZ (first real `customInsights`) → FL → GA → CA.
...
- **Phase 3 (UNBLOCKED 2026-06-29) — migrate the 5 remaining legacy bespoke pages (AZ, CA, FL, GA, TN)
  onto the v2 `[slug]` shell + delete the hand-written ~1,900-line clients.** Now low-risk: the v2 shell
  carries the full Design-D top-of-page via shared components (`state-top-of-page.tsx` + `state-verdict.ts`,
  PR #510), so migration is config-driven (build each state's `StateConfig` + competitive-landscape data,
  point the route at v2, delete the bespoke file). **Alabama is the design sandbox** — it keeps its bespoke
  `alabama-client.tsx` (Design D shipped via #507–#509) and should be the last to converge onto the shared
  components, or stay bespoke as the template; decide when Phase 3 starts.
