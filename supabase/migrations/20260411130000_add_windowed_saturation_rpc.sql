-- RPC: get_ad_saturation_windowed
-- Computes saturation metrics on the fly from ad_observations_raw
-- filtered by a date window. Replicates the scoring formula from
-- pipeline/pipelines/ad_intel_daily.py step_score():
--   saturation = (adv_ratio * 0.4 + spend_ratio * 0.35 + creative_ratio * 0.25) * 100
-- where each ratio is the group value divided by the max across all groups
