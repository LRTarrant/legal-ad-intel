...
// --- JPML SNAPSHOT QUERIES ---

export interface JpmlSnapshotRow {
  id: string;
  report_date: string;
  mdl_number: number;
  case_name: string;
...
export interface JpmlTypeSummaryRow {
  id: string;
  report_date: string;
  mdl_type: string;
  mdl_count: number;
...
export async function getLatestJpmlReportDate(): Promise<string | null> {
  const supabase = getSupabase();
  ...
  if (error) {
    console.error("getLatestJpmlReportDate error", error);
    return null;
  }
...
export async function getJpmlSnapshots(
  reportDate?: string | null
): Promise<{ reportDate: string | null; rows: JpmlSnapshotRow[] }> {
  const supabase = getSupabase();
  const targetDate = normalizeReportDate(reportDate) ?? (await getLatestJpmlReportDate());
  ...
  const { data, error } = await supabase
    .from("jpml_snapshots")
    .select(
      "id, report_date, mdl_number, case_name, jpml_type, transferee_judge, district, master_docket"
    )
  ...
  if (error) {
    console.error("getJpmlSnapshots error", error);
    return { reportDate: targetDate, rows: [] };
  }

  return {
    reportDate: targetDate,
    rows: (data ?? []) as JpmlSnapshotRow[],
  };
}

export async function getJpmlTypeSummaries(
  reportDate?: string | null
): Promise<{ reportDate: string | null; rows: JpmlTypeSummaryRow[] }> {
  const supabase = getSupabase();
  const targetDate = normalizeReportDate(reportDate) ?? (await getLatestJpmlReportDate());
  ...
  const { data, error } = await supabase
    ...
    .select(
      "id, report_date, mdl_type, mdl_count, pct_of_total, total_active_mdls"
    )
    .eq("report_date", targetDate)
  ...
  if (error) {
    console.error("getJpmlTypeSummaries error", error);
    return { reportDate: targetDate, rows: [] };
  }

  return {
    reportDate: targetDate,
    rows: (data ?? []) as JpmlTypeSummaryRow[],
  };
}