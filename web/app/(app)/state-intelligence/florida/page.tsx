...
import type { FloridaPageData } from "./florida-client";
import { getJudicialProfiles, type JudicialProfileRow } from "@/lib/queries/judicial";

const FloridaClient = nextDynamic(() => import("./florida-client").then((m) => m.FloridaClient));
...
export default async function FloridaStatePage() {
  ...
  const results = await Promise.allSettled([
    ...
    getJudicialProfiles("FL"),
    fetchStormCount(),
  ]);

  if (results[0].status === "fulfilled") accidentSummary = results[0].value;
  else console.error("[Florida] fetchAccidentSummary failed:", results[0].reason);

  if (results[1].status === "fulfilled") ruralUrban = results[1].value;
  else console.error("[Florida] fetchRuralUrbanComparison failed:", results[1].reason);

  if (results[2].status === "fulfilled") stormSummary = results[2].value;
  else console.error("[Florida] fetchStormSummary failed:", results[2].reason);

  if (results[3].status === "fulfilled") boatingSummary = results[3].value;
  else console.error("[Florida] fetchBoatingSummary failed:", results[3].reason);

  if (results[4].status === "fulfilled") piViability = results[4].value;
  else console.error("[Florida] fetchPIViability failed:", results[4].reason);

  if (results[5].status === "fulfilled") censusDemographics = results[5].value;
  else console.error("[Florida] fetchCensusDemographics failed:", results[5].reason);

  if (results[6].status === "fulfilled") msaDemographics = results[6].value;
  else console.error("[Florida] fetchMSADemographics failed:", results[6].reason);

  if (results[7].status === "fulfilled") judicialRows = results[7].value;
  else console.error("[Florida] fetchJudicialProfiles failed:", results[7].reason);

  if (results[8].status === "fulfilled") stormCount = results[8].value;
  else console.error("[Florida] fetchStormCount failed:", results[8].reason);

  const pageData: FloridaPageData = {
    accidentSummary,
    ruralUrban,
    stormSummary,
    boatingSummary,
  ...
  return <FloridaClient data={pageData} />;
}
