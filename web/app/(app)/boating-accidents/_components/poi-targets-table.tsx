...
import type { BoatingPoiTarget, BoatingPoiCategory } from "@/lib/queries";

type PoiTargetsTableProps = {
  pois: BoatingPoiTarget[];
  categories: BoatingPoiCategory[];
  selectedState: string | null;
};

const categoryColors: Record<string, string> = {
  marina: "bg-teal-100 text-teal-700",
  boat_ramp: "bg-blue-100 text-blue-700",
  marine_dealer: "bg-amber-100 text-amber-700",
  fuel_dock: "bg-rose-100 text-rose-700",
};
...
export function PoiTargetsTable({
  pois,
  categories,
  selectedState,
}: PoiTargetsTableProps) {
              ...
              {filtered.map((row, index) => (
                ...
                >
                  ...
                  <td className="py-2.5 pr-3 text-right font-mono text-midnight-navy">
                    {row.nearby_incidents.toLocaleString()}
                  </td>
                  <td className="py-2.5 pr-3 text-right font-mono text-midnight-navy">
                    {row.nearby_fatalities.toLocaleString()}
                  </td>
                  <td className="py-2.5 pr-3 text-right font-mono text-midnight-navy">
                    {row.nearby_injuries.toLocaleString()}
                  </td>
