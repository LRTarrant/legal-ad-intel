...
export async function generateMetadata({
  ...
  const tort = await getTortByUrlSlug(slug);
  if (!tort) return { title: "Not Found" };
  return {
    title: `${tort.label} Advertising Intelligence | Legal Marketing Intelligence`,
  };
}
...
/**
 * Catch-all dynamic route for tort advertising pages at /advertising/{slug}.
 * Torts that have their own dedicated page (e.g. social-media-addiction,
 * roblox-abuse, glp1-gastroparesis) are served by their static route instead.
 * This page handles all other torts that previously lived at /advertising/torts/{slug}.
 */
export default async function TortAdvertisingCatchallPage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
